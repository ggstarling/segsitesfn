  ...
  return (
    <section className="pt-4 pb-8 bg-muted/30">
      <div className="max-w-7xl mx-auto px-4">
        <h2 className="text-base font-medium text-center mb-6 text-gray-500">Parceiros que confiam em nosso trabalho</h2>
            ...
            {[...partners, ...partners, ...partners].map((partner, index) => (
              <div key={index} className="flex-shrink-0 flex items-center justify-center">
                <img
                  src={partner.logo}
                  alt={partner.name}
                  className="h-40 w-auto max-w-[350px] object-contain opacity-80 hover:opacity-100 transition-all duration-300 filter grayscale hover:grayscale-0 hover:scale-105"
                />
              </div>
            ))}
          </div>
